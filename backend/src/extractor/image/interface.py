...
class ImageExtractorInterface(ABC):
    """
    Interface for image extractors.
    Defines the contract that all image extractors must implement.
    """
    ...
    def read(self, file_path: str, **kwargs) -> dict:
        ...
        Content should contain extracted text/data.
        """
        pass
    ...
    def calculate_cost(self, image_count: int, api_response: Optional[dict] = None) -> float:
        """
        Calculate extraction cost based on image count and API response.
    ...
    def get_usage_metrics(self, api_response: Optional[dict] = None) -> dict:
        ...
        Returns:
            dict: Usage metrics (image_count, tokens, etc.)
        """
        pass