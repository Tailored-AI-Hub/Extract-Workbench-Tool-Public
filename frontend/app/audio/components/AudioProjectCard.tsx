...
import { FileAudio, Clock, Trash2, User } from 'lucide-react'
import { AudioProject } from '../../services/audioApi'
import { formatDate } from '../../pdf/utils/formatters'
import { ConfirmationDialog } from '../../components/ui/confirmation-dialog'

interface AudioProjectCardProps {
  project: AudioProject
  onDelete?: (project: AudioProject) => void
  deleting?: boolean
}

export function AudioProjectCard({ project, onDelete, deleting = false }: AudioProjectCardProps) {
  ...
  return (
    <>
      <Card className="h-full hover:shadow-md transition-shadow relative group">
        <Link href={`/audio/projects/${project.uuid}`} className="block h-full">
          <CardHeader>
      ...
      <ConfirmationDialog
        ...
        description={`Are you sure you want to delete "${project.name}"? This action cannot be undone and will delete all associated audio files and annotations.`}
        variant="destructive"
        confirmText="Delete"
        cancelText="Cancel"
        loading={deleting}
