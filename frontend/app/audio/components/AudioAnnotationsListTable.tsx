...
import { audioApi, AudioExtractionJob, AudioAnnotation } from '../../services/audioApi'
import { formatAudioExtractorName } from '../utils'

interface AudioAnnotationsListTableProps {
  projectId: string
  audioId: string
  token: string | null
  extractionJobs: AudioExtractionJob[]
  onAnnotationClick: (segmentNumber: number, extractorUuid: string, annotationUuid: string) => void
}

// Helper to find extractor name from job UUID
function findExtractorName(jobUuid: string, jobs: AudioExtractionJob[]): string {
  const job = jobs.find(j => j.uuid === jobUuid)
  return job?.extractor_display_name || job?.extractor || jobUuid
}

function extractSelectedText(annotation: AudioAnnotation): string {
...
export function AudioAnnotationsListTable({
  projectId,
  audioId,
  token,
  extractionJobs,
  onAnnotationClick,
}: AudioAnnotationsListTableProps) {
  ...
  useEffect(() => {
    const fetchAnnotations = async () => {
      ...
      try {
        ...
        const data = await audioApi.getAudioAnnotations(audioId, token, extractorUuid)
    ...
    fetchAnnotations()
  }, [audioId, token, selectedExtractor])
                ...
                return (
                  <TableRow
                    ...
                    onClick={() => onAnnotationClick(
                      annotation.segment_number,
                      annotation.extraction_job_uuid,
                      annotation.uuid
                    )}
                  >
                    <TableCell className="text-sm">{findExtractorName(annotation.extraction_job_uuid, extractionJobs)}</TableCell>
                    <TableCell className="text-sm text-muted-foreground">
                      {annotation.user_name || 'Unknown'}
                    </TableCell>
                    <TableCell className="max-w-md">
                      <div className="text-sm" title={selectedText || annotation.text}>
                        {truncateText(selectedText || annotation.text, 100)}
                      </div>
                    </TableCell>
