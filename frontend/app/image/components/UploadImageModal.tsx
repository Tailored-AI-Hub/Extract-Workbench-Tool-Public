...
interface UploadImageModalProps {
  isOpen: boolean
  onClose: () => void
  onSubmit: (files: File[], selectedExtractors: string[]) => void
  loading?: boolean
}

export default function UploadImageModal({ isOpen, onClose, onSubmit, loading = false }: UploadImageModalProps) {
  const { token } = useAuth()
  ...
  const { data: extractors, isLoading: extractorsLoading } = useQuery({
    queryKey: ['image-extractors'],
    queryFn: () => imageApi.getExtractors(token!),
    enabled: !!token && isOpen,
  })

  const relevantExtractors = (extractors?.image_extractors || [])
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    const validFiles = droppedFiles.filter((file) => {
      return /\.(jpg|jpeg|png|gif|bmp|tiff|tif|webp)$/i.test(file.name) || file.type.startsWith('image/')
    })
  ...
  const handleExtractorToggle = (extractorId: string) => {
    setSelectedExtractors((prev) =>
      prev.includes(extractorId) ? prev.filter((id) => id !== extractorId) : [...prev, extractorId]
    )
  }
  ...
  const handleSelectAllExtractors = () => {
    const allExtractorIds = relevantExtractors.flatMap((category) => category.extractors.map((extractor) => extractor.id))
  ...
  const handleSubmit = () => {
    if (files.length > 0 && selectedExtractors.length > 0) {
      onSubmit(files, selectedExtractors)
      setFiles([])
      setSelectedExtractors([])
      onClose()
    }
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Upload one or more image files and select extractors to process them.</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          >
            ...
            <p className="text-lg font-medium text-gray-900 mb-2">Drag and drop files here, or click to select</p>
            <p className="text-sm text-gray-500 mb-4">Supports image files (JPG, PNG, GIF, BMP, TIFF, WebP)</p>
            <input
              type="file"
              multiple
              accept=".jpg,.jpeg,.png,.gif,.bmp,.tiff,.tif,.webp,image/*"
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={handleSelectAllExtractors} disabled={extractorsLoading}>
                    Select All
                  </Button>
                  <Button variant="outline" size="sm" onClick={handleDeselectAllExtractors} disabled={extractorsLoading}>
                    Deselect All
                  </Button>
                </div>
              </div>
                    ...
                    <div key={category.category} className="border rounded-lg p-4">
                      ...
                      <div className="flex items-center justify-between mb-4 pb-2 border-b">
                        <div className="flex items-center space-x-3">
                          ...
                          <span className="text-xs text-gray-500">({category.extractors.length} extractors)</span>
                        </div>
                        ...
                        {category.extractors.map((extractor) => (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <label htmlFor={extractor.id} className="text-sm font-medium text-gray-900 cursor-pointer">
                                {extractor.name}
                              </label>
                              <div className="flex items-center gap-2 mt-1">
                                <span className="text-xs text-blue-600 font-medium">${extractor.cost_per_page.toFixed(4)}/image</span>
              ...
              {selectedExtractors.length > 0 && (
                <div className="mt-6 p-3 bg-blue-50 rounded-lg">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-semibold text-blue-900">Estimated cost: ${calculateTotalCost().toFixed(4)}/image</span>
                  </div>
                </div>
              )}
            </CardContent>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} disabled={files.length === 0 || selectedExtractors.length === 0 || loading} className="gap-2" type="button">
