...
import { imageApi, UserRatingBreakdown } from '../../services/imageApi'

interface ImageRatingBreakdownRowProps {
  projectId: string
  imageId: string
  jobUuid: string
  token: string | null
...
export function ImageRatingBreakdownRow({ projectId, imageId, jobUuid, token }: ImageRatingBreakdownRowProps) {
  ...
  useEffect(() => {
    const fetchBreakdown = async () => {
      ...
      try {
        setLoading(true)
        setError(null)
        const data = await imageApi.getImageRatingBreakdown(projectId, imageId, jobUuid, token)
        setBreakdown(data)
      } catch (err) {
        console.error('Error fetching image rating breakdown:', err)
    ...
    fetchBreakdown()
  }, [projectId, imageId, jobUuid, token])
