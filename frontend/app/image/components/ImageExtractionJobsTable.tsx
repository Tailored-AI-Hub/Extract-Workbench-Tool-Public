...
import { ImageRatingBreakdownRow } from './ImageRatingBreakdownRow'

type SortField = keyof ImageExtractionJob | null
type SortDirection = 'asc' | 'desc'

interface ImageExtractionJobsTableProps {
...
export function ImageExtractionJobsTable({ jobs, sortField, sortDirection, onSort, onViewExtractor, onRetryJob, retryingJobs = new Set(), projectId, imageId, token }: ImageExtractionJobsTableProps) {
  ...
  const renderSortIcon = (field: keyof ImageExtractionJob) =>
  ...
  const SortableHead = ({ field, label }: { field: keyof ImageExtractionJob; label: string }) => (
          ...
          jobs.map(job => {
            ...
            return (
              <React.Fragment key={job.uuid}>
                ...
                {isExpanded && projectId && imageId && (
                  <ImageRatingBreakdownRow
                    projectId={projectId}
                    imageId={imageId}
                    jobUuid={job.uuid}
                    token={token ?? null}
